...
async def update_card(
    ...
    return await card_service.update_card(db, current_user, card_id, payload)


@router.delete("/{card_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_card(card_id: UUID, db: DbSession, current_user: CurrentUser) -> Response:
    await card_service.delete_card(db, current_user, card_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.post("/{card_id}/move", response_model=CardPublic)
async def move_card(
    card_id: UUID, payload: CardMove, db: DbSession, current_user: CurrentUser
) -> CardPublic:
    return await card_service.move_card(db, current_user, card_id, payload)
